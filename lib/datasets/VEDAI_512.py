...
import subprocess
import uuid
from VEDAI_512_eval import VEDAI_512_eval
import collections
from fast_rcnn.config import cfg


class VEDAI_512(imdb):

    def __init__(self, image_set, devkit_path=None):
        imdb.__init__(self, 'VEDAI_512' + '_' + image_set)
        ...
        assert os.path.exists(self._devkit_path), \
            'VEDAI_512 path does not exist: {}'.format(self._devkit_path)
    ...
    def _get_default_path(self):
        """
        Return the default path where VEDAI_512Person is expected to be installed.
        You need to add a link in the data folder to VEDAI_512Person datasets folder
        """
        return os.path.join(cfg.DATA_DIR, 'VEDAI_512')
    ...
    def gt_roidb(self):
        ...
        gt_roidb = [self._load_VEDAI_512_annotation(index)
    ...
    def _load_selective_search_roidb(self, gt_roidb):
        base_path = os.path.join(cfg.DATA_DIR, 'VEDAI_512')
        if self._image_set == 'Test':
    ...
    def _load_VEDAI_512_annotation(self, index):
        """
    ...
    def _get_VEDAI_512_results_file_template(self):
    ...
    def _write_VEDAI_512_results_file(self, all_boxes):
        #print('Debug -- Shape of all_boxes {}'.format(all_boxes))
        for cls_ind, cls in enumerate(self.classes):
            if cls == '__background__':
                continue
            print 'Writing {} VEDAI_512 results file'.format(cls)
            filename = self._get_VEDAI_512_results_file_template().format(cls)
    ...
    def _do_python_eval(self, output_dir='output'):
        print '--------------------------------------------------------------'
        print 'Computing results for VEDAI_512Person dataset.'
        print '--------------------------------------------------------------'
    ...
    def evaluate_detections(self, all_boxes, output_dir):
        self._write_VEDAI_512_results_file(all_boxes)
...
if __name__ == '__main__':
    from datasets.VEDAI_512 import VEDAI_512
    d = VEDAI_512('Train')
    res = d.roidb
    from IPython import embed
    embed()
